...
- `codex:rescue` is **user-only** — it carries `disable-model-invocation`, so no subagent can start a write-capable Codex run. If work should be handed to Codex, say so in your report and let the main thread raise it with the user.
